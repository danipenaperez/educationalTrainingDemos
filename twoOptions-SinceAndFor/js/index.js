...
function fillInitlWithRandomValues(excluded){
    ...
    for(i=0;i<availableOptions;i++){
        var index = getRandomItem('excludedCorrectValues', excluded, false);
...
function getRandomItem(strategy, excluded, replaceForNewValueIfExists){
    ...
    if('excludedCorrectValues' == strategy){
        while (true) {
            ...
            if (!excluded.includes(options[candidateIndex].correct)){
                ...
                if(replaceForNewValueIfExists){
                    excluded = excluded.filter(function(item) {
                        return item !== options[candidateIndex].correct;
                    });
                    break;
                }
            }
...
function reload(excluded){
    var opt_index = getRandomItem('excludedCorrectValues', excluded, true);
